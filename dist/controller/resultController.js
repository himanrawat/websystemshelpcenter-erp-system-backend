...
const createResults = async (req, res) => {
    ...
    try {
        const newResults = [];
        for (const result of results) {
            const existingResult = await db_config_1.default.marks.findFirst({
                where: {
            ...
            const newResult = await db_config_1.default.marks.create({
                data: {
...
const updateResultPartial = async (req, res) => {
    ...
    try {
        ...
        const updatedResult = await db_config_1.default.marks.update({
...
const updateFullResult = async (req, res) => {
    const studentId = Number(req.params.studentId);
...
const getAllResults = async (req, res) => {
    try {
        const results = await db_config_1.default.marks.findMany({
            include: {
                student: {
                    include: {
                        campus: true,
...
const deleteResultPartial = async (req, res) => {
    const resultId = Number(req.params.resultId);
    try {
        await db_config_1.default.marks.delete({
            where: { id: resultId },
        });
...
const deleteResults = async (req, res) => {
    const studentId = Number(req.params.studentId);
    try {
        await db_config_1.default.marks.deleteMany({
            where: { studentId },
        });
