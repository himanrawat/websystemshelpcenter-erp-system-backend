...
const createHoliday = async (req, res) => {
    ...
    try {
        ...
        const isHoliday = await db_config_1.default.holiday.findUnique({
            where: {
                date: date,
            },
        });
        ...
        const holiday = await db_config_1.default.holiday.create({
            data: {
                date: new Date(date),
                reason,
                teacherId,
...
const getHoliday = async (req, res) => {
    ...
    try {
        const holidays = await db_config_1.default.holiday.findMany({
            orderBy: {
                date: "asc",
            },
            where: {
...
const getHolidayOnDate = async (req, res) => {
    const givenDate = req.params.date;
    const campusId = parseInt(req.params.campusId);
    try {
        const date = new Date(givenDate);
        const holiday = await db_config_1.default.holiday.findUnique({
            where: {
                date: date,
                campusId: campusId
            }
...
const updateHoliday = async (req, res) => {
    ...
    try {
        ...
        const holiday = await db_config_1.default.holiday.update({
            where: {
                id: holiday_id
            },
            data: {
...
const deleteHoliday = async (req, res) => {
    ...
    try {
        ...
        await db_config_1.default.holiday.delete({
            where: {
                id: holiday_id,
            },
        });
